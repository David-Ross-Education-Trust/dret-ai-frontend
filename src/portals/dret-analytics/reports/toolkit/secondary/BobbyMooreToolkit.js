...
import { BobbyMooreConfig } from "../toolkit/BobbyMooreConfig";

const TRUST_GREEN = "#205c40";

function useFavourites(key = "toolkitFavourites_BobbyMoore") {
...
export default function BobbyMooreToolkit() {
  ...
  const [favourites, toggleFavourite] = useFavourites("toolkitFavourites_BobbyMoore");
  const [clickedStar, setClickedStar] = useState(null);

  const shownFiles = BobbyMooreConfig.filter(
    (file) =>
      searchTerm.trim() === "" ||
