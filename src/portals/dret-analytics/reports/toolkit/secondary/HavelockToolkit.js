...
import { HavelockConfig } from "./HavelockConfig";

const TRUST_GREEN = "#205c40";

function useFavourites(key = "toolkitFavourites_Havelock") {
...
export default function HavelockToolkit() {
  ...
  const [favourites, toggleFavourite] = useFavourites("toolkitFavourites_Havelock");
  const [clickedStar, setClickedStar] = useState(null);

  const shownFiles = HavelockConfig.filter(
    (file) =>
      searchTerm.trim() === "" ||
