...
import { CharlesReadConfig } from "./CharlesReadConfig";

const TRUST_GREEN = "#205c40";

function useFavourites(key = "toolkitFavourites_CharlesRead") {
...
export default function CharlesReadToolkit() {
  ...
  const [favourites, toggleFavourite] = useFavourites("toolkitFavourites_CharlesRead");
  const [clickedStar, setClickedStar] = useState(null);

  const shownFiles = CharlesReadConfig.filter(
    (file) =>
      searchTerm.trim() === "" ||
