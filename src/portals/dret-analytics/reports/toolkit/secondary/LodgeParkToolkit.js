...
import { LodgeParkConfig } from "./LodgeParkConfig";

const TRUST_GREEN = "#205c40";

function useFavourites(key = "toolkitFavourites_LodgePark") {
...
export default function LodgeParkToolkit() {
  ...
  const [favourites, toggleFavourite] = useFavourites("toolkitFavourites_LodgePark");
  const [clickedStar, setClickedStar] = useState(null);

  const shownFiles = LodgeParkConfig.filter(
    (file) =>
      searchTerm.trim() === "" ||
