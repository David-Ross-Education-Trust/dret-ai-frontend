...
import { KingEdwardConfig } from "./KingEdwardConfig";

const TRUST_GREEN = "#205c40";

function useFavourites(key = "toolkitFavourites_KEVI") {
...
export default function KEVIToolkit() {
  const [searchTerm, setSearchTerm] = useState("");
  const [searchFocused, setSearchFocused] = useState(false);
  const [favourites, toggleFavourite] = useFavourites("toolkitFavourites_KEVI");
  const [clickedStar, setClickedStar] = useState(null);

  const shownFiles = KingEdwardConfig.filter(
    (file) =>
      searchTerm.trim() === "" ||
