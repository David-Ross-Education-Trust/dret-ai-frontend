...
import { AinthorpeConfig } from "./AinthorpeConfig";

const TRUST_GREEN = "#205c40";

function useFavourites(key = "toolkitFavourites_Ainthorpe") {
...
export default function AinthorpeToolkit() {
  ...
  const [favourites, toggleFavourite] = useFavourites("toolkitFavourites_Ainthorpe");
  const [clickedStar, setClickedStar] = useState(null);

  const shownFiles = AinthorpeConfig.filter(
    (file) =>
      searchTerm.trim() === "" ||
