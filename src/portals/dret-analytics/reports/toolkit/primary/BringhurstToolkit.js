...
import { BringhurstConfig } from "./BringhurstConfig";

const TRUST_GREEN = "#205c40";

function useFavourites(key = "toolkitFavourites_Bringhurst") {
...
export default function BringhurstToolkit() {
  ...
  const [favourites, toggleFavourite] = useFavourites(
    "toolkitFavourites_Bringhurst"
  );
  const [clickedStar, setClickedStar] = useState(null);

  const shownFiles = BringhurstConfig.filter(
    (file) =>
      searchTerm.trim() === "" ||
