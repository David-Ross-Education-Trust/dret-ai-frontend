...
import { HogsthorpeConfig } from "./HogsthorpeConfig";

const TRUST_GREEN = "#205c40";

function useFavourites(key = "toolkitFavourites_Hogsthorpe") {
...
export default function HogsthorpeToolkit() {
  const [searchTerm, setSearchTerm] = useState("");
  const [searchFocused, setSearchFocused] = useState(false);
  const [favourites, toggleFavourite] = useFavourites(
    "toolkitFavourites_Hogsthorpe"
  );
  const [clickedStar, setClickedStar] = useState(null);

  const shownFiles = HogsthorpeConfig.filter(
    (file) =>
      searchTerm.trim() === "" ||
