...
import { RockinghamConfig } from "./RockinghamConfig";

const TRUST_GREEN = "#205c40";

function useFavourites(key = "toolkitFavourites_Rockingham") {
...
export default function RockinghamToolkit() {
  const [searchTerm, setSearchTerm] = useState("");
  const [searchFocused, setSearchFocused] = useState(false);
  const [favourites, toggleFavourite] = useFavourites(
    "toolkitFavourites_Rockingham"
  );
  const [clickedStar, setClickedStar] = useState(null);

  const shownFiles = RockinghamConfig.filter(
    (file) =>
      searchTerm.trim() === "" ||
