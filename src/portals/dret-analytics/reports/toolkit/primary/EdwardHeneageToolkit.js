...
import { EdwardHeneageConfig } from "./EdwardHeneageConfig";

const TRUST_GREEN = "#205c40";

function useFavourites(key = "toolkitFavourites_EdwardHeneage") {
...
export default function EdwardHeneageToolkit() {
  const [searchTerm, setSearchTerm] = useState("");
  const [searchFocused, setSearchFocused] = useState(false);
  const [favourites, toggleFavourite] = useFavourites(
    "toolkitFavourites_EdwardHeneage"
  );
  const [clickedStar, setClickedStar] = useState(null);

  const shownFiles = EdwardHeneageConfig.filter(
    (file) =>
      searchTerm.trim() === "" ||
