...
import { KingsHeathConfig } from "./KingsHeathConfig";

const TRUST_GREEN = "#205c40";

function useFavourites(key = "toolkitFavourites_KingsHeath") {
...
export default function KingsHeathToolkit() {
  ...
  const [favourites, toggleFavourite] = useFavourites(
    "toolkitFavourites_KingsHeath"
  );
  const [clickedStar, setClickedStar] = useState(null);

  const shownFiles = KingsHeathConfig.filter(
    (file) =>
      searchTerm.trim() === "" ||
