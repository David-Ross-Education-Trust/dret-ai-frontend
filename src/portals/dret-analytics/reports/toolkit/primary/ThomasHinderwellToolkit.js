...
import { ThomasHinderwellConfig } from "./ThomasHinderwellConfig";

const TRUST_GREEN = "#205c40";

function useFavourites(key = "toolkitFavourites_ThomasHinderwell") {
...
export default function ThomasHinderwellToolkit() {
  ...
  const [favourites, toggleFavourite] = useFavourites(
    "toolkitFavourites_ThomasHinderwell"
  );
  const [clickedStar, setClickedStar] = useState(null);

  const shownFiles = ThomasHinderwellConfig.filter(
    (file) =>
      searchTerm.trim() === "" ||
