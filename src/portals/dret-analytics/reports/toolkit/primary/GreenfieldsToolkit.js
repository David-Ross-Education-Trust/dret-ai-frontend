...
import { GreenfieldsConfig } from "./GreenfieldsConfig";

const TRUST_GREEN = "#205c40";

function useFavourites(key = "toolkitFavourites_Greenfields") {
...
export default function GreenfieldsToolkit() {
  const [searchTerm, setSearchTerm] = useState("");
  const [searchFocused, setSearchFocused] = useState(false);
  const [favourites, toggleFavourite] = useFavourites(
    "toolkitFavourites_Greenfields"
  );
  const [clickedStar, setClickedStar] = useState(null);

  const shownFiles = GreenfieldsConfig.filter(
    (file) =>
      searchTerm.trim() === "" ||
