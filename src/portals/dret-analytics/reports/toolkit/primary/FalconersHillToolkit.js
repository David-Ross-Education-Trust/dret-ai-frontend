...
import { FalconersHillConfig } from "./FalconersHillConfig";

const TRUST_GREEN = "#205c40";

function useFavourites(key = "toolkitFavourites_FalconersHill") {
...
export default function FalconersHillToolkit() {
  const [searchTerm, setSearchTerm] = useState("");
  const [searchFocused, setSearchFocused] = useState(false);
  const [favourites, toggleFavourite] = useFavourites(
    "toolkitFavourites_FalconersHill"
  );
  const [clickedStar, setClickedStar] = useState(null);

  const shownFiles = FalconersHillConfig.filter(
    (file) =>
      searchTerm.trim() === "" ||
