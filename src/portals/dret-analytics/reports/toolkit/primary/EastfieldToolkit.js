...
import { EastfieldConfig } from "./EastfieldConfig";

const TRUST_GREEN = "#205c40";

function useFavourites(key = "toolkitFavourites_Eastfield") {
...
export default function EastfieldToolkit() {
  const [searchTerm, setSearchTerm] = useState("");
  const [searchFocused, setSearchFocused] = useState(false);
  const [favourites, toggleFavourite] = useFavourites(
    "toolkitFavourites_Eastfield"
  );
  const [clickedStar, setClickedStar] = useState(null);

  const shownFiles = EastfieldConfig.filter(
    (file) =>
      searchTerm.trim() === "" ||
