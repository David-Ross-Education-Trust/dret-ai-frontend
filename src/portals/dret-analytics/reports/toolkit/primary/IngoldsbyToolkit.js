...
import { IngoldsbyConfig } from "./IngoldsbyConfig";

const TRUST_GREEN = "#205c40";

function useFavourites(key = "toolkitFavourites_Ingoldsby") {
...
export default function IngoldsbyToolkit() {
  ...
  const [favourites, toggleFavourite] = useFavourites(
    "toolkitFavourites_Ingoldsby"
  );
  const [clickedStar, setClickedStar] = useState(null);

  const shownFiles = IngoldsbyConfig.filter(
    (file) =>
      searchTerm.trim() === "" ||
