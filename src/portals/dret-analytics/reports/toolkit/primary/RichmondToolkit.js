...
import { RichmondConfig } from "./RichmondConfig";

const TRUST_GREEN = "#205c40";

function useFavourites(key = "toolkitFavourites_Richmond") {
...
export default function RichmondToolkit() {
  ...
  const [favourites, toggleFavourite] = useFavourites(
    "toolkitFavourites_Richmond"
  );
  const [clickedStar, setClickedStar] = useState(null);

  const shownFiles = RichmondConfig.filter(
    (file) =>
      searchTerm.trim() === "" ||
