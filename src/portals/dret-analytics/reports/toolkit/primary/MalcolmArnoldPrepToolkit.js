...
import { MalcolmArnoldPrepConfig } from "./MalcolmArnoldPrepConfig";

const TRUST_GREEN = "#205c40";

function useFavourites(key = "toolkitFavourites_MalcolmArnoldPrep") {
...
export default function MalcolmArnoldPrepToolkit() {
  ...
  const [favourites, toggleFavourite] = useFavourites(
    "toolkitFavourites_MalcolmArnoldPrep"
  );
  const [clickedStar, setClickedStar] = useState(null);

  const shownFiles = MalcolmArnoldPrepConfig.filter(
    (file) =>
      searchTerm.trim() === "" ||
