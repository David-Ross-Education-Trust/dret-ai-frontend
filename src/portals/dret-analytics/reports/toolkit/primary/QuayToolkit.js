...
import { QuayConfig } from "./QuayConfig";

const TRUST_GREEN = "#205c40";

function useFavourites(key = "toolkitFavourites_Quay") {
...
export default function QuayToolkit() {
  ...
  const [favourites, toggleFavourite] = useFavourites("toolkitFavourites_Quay");
  const [clickedStar, setClickedStar] = useState(null);

  const shownFiles = QuayConfig.filter(
    (file) =>
      searchTerm.trim() === "" ||
