...
import { TheArboursConfig } from "./TheArboursConfig";

const TRUST_GREEN = "#205c40";

function useFavourites(key = "toolkitFavourites_TheArbours") {
...
export default function TheArboursToolkit() {
  const [searchTerm, setSearchTerm] = useState("");
  const [searchFocused, setSearchFocused] = useState(false);
  const [favourites, toggleFavourite] = useFavourites(
    "toolkitFavourites_TheArbours"
  );
  const [clickedStar, setClickedStar] = useState(null);

  const shownFiles = TheArboursConfig.filter(
    (file) =>
      searchTerm.trim() === "" ||
