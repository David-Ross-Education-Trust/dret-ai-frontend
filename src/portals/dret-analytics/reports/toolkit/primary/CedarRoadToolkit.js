...
import { CedarRoadConfig } from "./CedarRoadConfig";

const TRUST_GREEN = "#205c40";

function useFavourites(key = "toolkitFavourites_CedarRoad") {
...
export default function CedarRoadToolkit() {
  const [searchTerm, setSearchTerm] = useState("");
  const [searchFocused, setSearchFocused] = useState(false);
  const [favourites, toggleFavourite] = useFavourites(
    "toolkitFavourites_CedarRoad"
  );
  const [clickedStar, setClickedStar] = useState(null);

  const shownFiles = CedarRoadConfig.filter(
    (file) =>
      searchTerm.trim() === "" ||
