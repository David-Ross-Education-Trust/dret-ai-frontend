...
import { BoothWoodConfig } from "./BoothWoodConfig";

const TRUST_GREEN = "#205c40";

function useFavourites(key = "toolkitFavourites_BoothWood") {
...
export default function BoothWoodToolkit() {
  ...
  const [favourites, toggleFavourite] = useFavourites("toolkitFavourites_BoothWood");
  const [clickedStar, setClickedStar] = useState(null);

  const shownFiles = BoothWoodConfig.filter(
    (file) =>
      searchTerm.trim() === "" ||
