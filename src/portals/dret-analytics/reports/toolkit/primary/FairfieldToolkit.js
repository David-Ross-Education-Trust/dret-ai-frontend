...
import { FairfieldConfig } from "./FairfieldConfig";

const TRUST_GREEN = "#205c40";

function useFavourites(key = "toolkitFavourites_Fairfield") {
...
export default function FairfieldToolkit() {
  ...
  const [favourites, toggleFavourite] = useFavourites(
    "toolkitFavourites_Fairfield"
  );
  const [clickedStar, setClickedStar] = useState(null);

  const shownFiles = FairfieldConfig.filter(
    (file) =>
      searchTerm.trim() === "" ||
