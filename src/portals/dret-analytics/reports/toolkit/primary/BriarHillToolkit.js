...
import { BriarHillConfig } from "./BriarHillConfig";

const TRUST_GREEN = "#205c40";

function useFavourites(key = "toolkitFavourites_BriarHill") {
...
export default function BriarHillToolkit() {
  const [searchTerm, setSearchTerm] = useState("");
  const [searchFocused, setSearchFocused] = useState(false);
  const [favourites, toggleFavourite] = useFavourites(
    "toolkitFavourites_BriarHill"
  );
  const [clickedStar, setClickedStar] = useState(null);

  const shownFiles = BriarHillConfig.filter(
    (file) =>
      searchTerm.trim() === "" ||
