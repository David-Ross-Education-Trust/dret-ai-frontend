...
export default function ToolTemplate() {
  ...
  // 3. Update this to handle any new fields you add/remove
  const handleChange = (name, value) => {
    setFields((prev) => ({ ...prev, [name]: value }));
  };

  // 4. Update the handleSubmit logic as needed for new fields and validation
  const handleSubmit = async (e) => {
    e.preventDefault();

    // Update this check if you change required fields
    if (
      !fields.subject.trim() ||
    ...
    // 5. Change this message template to what you want to send to your backend/AI
    // You can include/exclude whatever fields you wish.
    const message = `
...
Add any prompt wording or formatting instructions here.
    ...
    try {
      const res = await fetch("https://dret-ai-backend-f9drcacng0f2gmc4.uksouth-01.azurewebsites.net/ask", {
        ...
        body: JSON.stringify({
          agentId: "asst_1UapHOvlRnY8SYKyizeXmUcZ", // <<< REPLACE with your new agentId!
          message,
        }),
      });
  ...
  // 7. Export logic should usually be the same
  const exportToDocx = () => {
    ...
    link.href = url;
    link.download = "exported.docx"; // Optional: Change the filename if you want
  ...
  // 8. Update form inputs to match your fields (add/remove fields as needed)
  return (
    <Layout>
      <div className="font-avenir bg-gray-50 min-h-screen h-screen flex flex-col">
        ...
        <div className="flex flex-1 min-h-0 w-full gap-8 px-8 py-8 bg-gray-100">
          ...
          >
            {/* 10. Copy/paste and update these <div>s to match your fields */}
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">Field 1 Label</label>
              <input
                type="text"
              ...
              >
                {loading ? (
                  <div className="flex flex-col items-center justify-center h-full w-full py-10">
                    ...
                    <div className="text-gray-500 italic text-sm mt-3">Generating...</div>
                  </div>
                ) : response ? (
